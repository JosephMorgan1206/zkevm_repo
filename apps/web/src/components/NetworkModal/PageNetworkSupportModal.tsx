  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a zkEVM Chain only feature')}</Text>
        ...
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.ETHEREUM) : switchNetworkAsync(ChainId.ETHEREUM))}
          >
            {t('Switch to %chain%', { chain: 'Polygon zkEVM Chain' })}
          </Button>
        ) : (
